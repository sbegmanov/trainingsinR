...
polling_places %>% 
  ...
  geom_line(alpha = 0.8, linewidth = 1.2, show.legend = FALSE) +
...
```{r}
total_change <- by_state %>% 
  group_by(state) %>% 
  mutate(change = n - lag(n)) %>% 
  filter(n() > 1) %>% 
  summarize(change = sum(change, na.rm = TRUE) / first(n)) # delete by first in group
...
us_states %>% 
  inner_join(tibble(state = state.abb, region = str_to_lower(state.name))) %>% left_join(total_change) %>% 
  ggplot(aes(x = long, y = lat, group = group, fill = change)) +
  geom_polygon(color = "black", linewidth = 0.1) +
  coord_map() +
  scale_fill_distiller(palette = "RdYlBu", direction = 1, na.value = "gray95", limits = limits, label = scales::label_percent()) +
